...
<?php
  ...
  if(isset($_GET["genero"])) {
	...
	$sql = mysql_query("SELECT * FROM musicas WHERE pontos >= 0 AND categoria = 'djset' AND genero = '$genero' ORDER BY pontos DESC LIMIT 10"); 
	 
 $genero = str_replace("_"," ", $genero);
 $genero = str_replace("5","/", $genero);
 $genero = str_replace("8","&", $genero);
	   
	  $resultado = "Resultado de todas os DjSet no gênero ".$genero."!";
	   $resultadoTop10 = "Resultado do Top 10 com DjSet no gênero ".$genero."!";
  }else{
  $sql = mysql_query("SELECT * FROM musicas WHERE pontos >= 0 AND categoria = 'djset' ORDER BY pontos DESC LIMIT 10");
	  $resultado = "DjSets";
	  $resultadoTop10 = "Top 10 DjSets";
  }
...
<a id="ln" href="top100.php?top=djset" style="margin:4px 0px 0px 0px;float:right;color:#EEE;">
+ Top 100
</a>
...
if($existe == 0){
	...
	echo '<p id="nome" style="margin:10px 0px -25px 0px;width:100%;color:#EEE;">Não existe nenhum resultado para a categoria DjSet!</p>';
...
<?php
  ...
  if(isset($_GET["genero"])) {
	$genero = $_GET['genero'];
    $sql = mysql_query("SELECT * FROM musicas WHERE categoria='djset' AND genero='$genero'");
  }else{
    $sql = mysql_query("SELECT * FROM musicas WHERE categoria='djset' ORDER BY data DESC");
  }

 $djsetGET = str_replace("_"," ", $djsetGET);
 $djsetGET = str_replace("5","/", $djsetGET);
 $djsetGET = str_replace("8","&", $djsetGET);
$existe = mysql_num_rows($sql);
if($existe == 0){
	echo '<p id="nome" style="width:100%;color:#EEE;">Não existe nenhum resultado para a categoria DjSet!</p>';
}else{
...
<div id="img_music">
<a id="player" href="player.php?musica='.$track->id_musica.'">
...
<a id="download01" target="_blank" href="sounds/'.$tracks->url_musica.'" style="background:#f50044 url(img/download01.png) no-repeat center center;"></a>
